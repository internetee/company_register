...
class CompanyRegisterTest < Minitest::Test
  ...
  def stub_wsdl_request
    ...
    stub_request(:get, CompanyRegister.configuration.wsdl).to_return(status: 200,
                                                                     body: response_body)
  end

  def stub_request_with_payload
    response_body = File.read('test/fixtures/representation_rights_response_with_payload.xml')
    stub_request(:post, CompanyRegister.configuration.endpoint).
        with(
  ...
  def stub_request_without_payload
    ...
    stub_request(:post, CompanyRegister.configuration.endpoint).to_return(status: 200,
                                                                          body: response_body)
  end
end